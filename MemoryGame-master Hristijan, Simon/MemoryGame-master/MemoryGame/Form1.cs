...
namespace MemoryGame
{
    public partial class Form1 : Form
    {
        ...
        void resetImages()
        {
            ...
            // gi inicijalizira site picbox sliki vo ?
        }
        ...
        void resetTags()
        {
            ...
            // gi inicijalizira site picbox tagovi na 0
        }
        ...
        void showImage(PictureBox box)
        {
            ...
            // Ovaa funkcija konvertira picbox tag vo image;
        ...
        void setTagRandom()
        {
          ...
          /* Funkcijata ima 3 cekora.
           * 1) Pravi niza i ja popolnuva vnatresnosta so random brojki pomegju 1 i 16
           * 2) Pravi kratenje na broevi pogolemi od 8, zosto imame 8 razlicni sliki za igrata.
           * 3) Gi mesti brojkite vo nizata kako tagovi na picboxovite.
           */
        }
        void compare(PictureBox previous, PictureBox current)
        {
            if(previous.Tag.ToString()==current.Tag.ToString())
            {
                ...
                if(--remain==0)
                {
                    ...
                    remaining.Text = "Честитки.";
                    MessageBox.Show("Честитки.Ја завршивте играта на " + timeLeft + " секунди пред крајот." , "Крај на играта.");
                    Hint.Enabled = false;
                }
               else remaining.Text = "Преостанати картички: " + remain;
            }
            else
            {
                Application.DoEvents();
        ...
        void newgame()
        {
            ...
            resetImages();
            Hint.Enabled = true;
            remaining.Text = "Преостанати картички: " + remain;
            Hint.Text = "Помош (" + hint + ")";
            flag = 0;
            timeLeft = 60;
            time.Text = "Време преостанато: " + timeLeft;
            timer1.Enabled = true;
            activeAll();
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (--timeLeft == 0)
            {
                ...
                time.Text = "Времето истече.";
                MessageBox.Show("Времето истече", "Играта заврши!");
                deActiveAll();
                Hint.Enabled = false;
                
            }
            else
            time.Text = "Преостанато време: " + timeLeft;
